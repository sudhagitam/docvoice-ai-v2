...
import { AlertCircle, Mic, RotateCcw, BookOpen } from "lucide-react";
    ...
    <main className="min-h-screen flex flex-col">
      ...
      <header className="border-b border-ink-800/60 backdrop-blur-sm sticky top-0 z-50 bg-ink-950/80">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          {/* ── Mode toggle ── */}
          <div className="flex items-center gap-1 bg-ink-900/80 border border-ink-800 rounded-xl p-1">
            <button
              ...
              className={`
                ...
                ${mode === "classic"
                  ? "bg-voice text-ink-950 shadow-[0_0_12px_rgba(62,207,207,0.3)]"
              ...
              <div className="lg:col-span-3 space-y-5">
                ...
                {dv.stage === "idle" && (
                  <div className="glass rounded-2xl p-12 flex flex-col items-center justify-center text-center min-h-[300px]">
                    ...
                    <button
                      onClick={() => setMode("reader")}
                      className="mt-6 flex items-center gap-2 px-4 py-2 rounded-xl border border-ink-700 text-ink-400 hover:text-voice hover:border-voice/40 text-sm font-display font-semibold transition-all"
                    >
                      <BookOpen className="w-4 h-4" />
                      Try Page Reader for large PDFs
                    </button>
                  </div>
                )}
